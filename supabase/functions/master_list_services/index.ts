...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await db
      .from("services")
      .select("id, title, duration_min, price_rub, is_active")
      .eq("master_id", master.id)
      .order("title", { ascending: true });

    if (error) throw error;

    return new Response(JSON.stringify({ ok: true, services: data ?? [] }), { headers: { ...corsHeaders, "content-type": "application/json" } });
  } catch (e) {
